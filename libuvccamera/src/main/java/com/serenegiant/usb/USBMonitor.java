...
//		if (DEBUG) Log.v(TAG, "requestPermission:device=" + device);
	...
	public void checkUSB(){
		...
		while (it.hasNext()) {
			...
			//if (mUsbManager.hasPermission(device) && device.getVendorId()==7758) {
			if (mUsbManager.hasPermission(device)&& device.getDeviceClass()==239) {

				Log.e(TAG,"Process Connecte from checkUSB");
				processConnect(device);
				return;
			} else {
				Log.e(TAG,"Does Not Have Permission ");
				//PendingIntent mPermissionIntent = PendingIntent.getBroadcast(mWeakContext.get(), 0, new Intent(ACTION_USB_PERMISSION), 0);
				//mUsbManager.requestPermission(device, mPermissionIntent);
			}
		}
	}
	...
	private final BroadcastReceiver mUsbReceiver = new BroadcastReceiver() {
		...
		public void onReceive(final Context context, final Intent intent) {
			...
			if (ACTION_USB_PERMISSION.equals(action)) {
				...
				synchronized (USBMonitor.this) {
					...
					if (intent.getBooleanExtra(UsbManager.EXTRA_PERMISSION_GRANTED, false)) {
						if (device != null) {
							...
							Log.v(TAG,"Process connect for permission granted.");
							processConnect(device);
						}
					} else {
				...
				if(device!=null){
					Log.e(TAG,"Device Class: " + device.getDeviceClass());
					Log.e(TAG,"Device Subclass: " + device.getDeviceSubclass());
				}else{
	...
	private final void processConnect(final UsbDevice device) {
		if (destroyed){
			if (DEBUG) Log.v(TAG, "processConnect:destroyed=" + device);
			return;
		}
